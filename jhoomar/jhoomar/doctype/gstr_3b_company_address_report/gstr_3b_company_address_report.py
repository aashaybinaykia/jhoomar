...
class GSTR3BCompanyAddressReport(Document):
	...
	def get_data(self):
		...
		companies = frappe.get_list(
			"Dynamic Link", 
			filters={
				'parent': self.company_address,
				'parenttype': 'Address',
				'link_doctype': 'Company'
			}, fields=('link_name'))

		self.companies = [row['link_name'] for row in companies]
	...
	def get_total_taxable_value(self, doctype, reverse_charge):
		...
		return frappe._dict(frappe.db.sql("""
			...
			""" #nosec
			.format(doctype = doctype), (self.month_no, self.year, reverse_charge, self.gst_details.get("gstin"))))
	...
	def get_itc_details(self):
		itc_amount = frappe.db.sql("""
			...
			""",
			(self.month_no, self.year, self.gst_details.get("gstin")), as_dict=1)
	...
	def get_nil_rated_supply_value(self):
		...
		return frappe.db.sql("""
			...
			and month(s.posting_date) = %s and year(s.posting_date) = %s
			and s.company_gstin = %s""",
			(self.month_no, self.year, self.gst_details.get("gstin")), as_dict=1)[0].total
	...
	def get_inter_state_supplies(self, state_number):
		inter_state_supply_tax = frappe.db.sql(""" select t.account_head, t.tax_amount_after_discount_amount as tax_amount,
			...
			and s.company_gstin = %s and s.gst_category in ('Unregistered', 'Registered Composition', 'UIN Holders')
		""", (self.month_no, self.year, self.gst_details.get("gstin")), as_dict=1)
	...
	def get_inward_nil_exempt(self, state):
		...
		inward_nil_exempt = frappe.db.sql(""" select p.place_of_supply, sum(i.base_amount) as base_amount,
			...
			month(p.posting_date) = %s and year(p.posting_date) = %s and p.company in (%s) and p.company_gstin = %s
			group by p.place_of_supply """, (self.month_no, self.year, ", ".join(self.companies) ,self.gst_details.get("gstin")), as_dict=1)
	...
	def get_tax_amounts(self, doctype, reverse_charge="N"):
		...
		tax_amounts = frappe.db.sql("""
			...
			(reverse_charge, self.month_no, self.year, self.gst_details.get("gstin")), as_dict=1)
	...
	def get_account_heads(self):
		...
		if account_heads:
			...
			frappe.throw(_("Please set account heads in GST Settings for Compnay {0}").format(", ".join(self.companies)))
	...
	def get_missing_field_invoices(self):
		...
		for doctype in ["Sales Invoice", "Purchase Invoice"]:
			...
			docnames = frappe.db.sql("""
				...
				and t1.company in (%s) and t1.place_of_supply IS NULL and t1.{party} = t2.name and
				t2.gst_category != 'Overseas'
			""".format(doctype = doctype, party_type = party_type, party=party), (self.month_no, self.year, ", ".join(self.companies)), as_dict=1) #nosec
...
def view_report(name):
	...
	json_data = frappe.get_value("GSTR 3B Company Address Report", name, 'json_output')
