...
def execute(filters=None):
	return Gstr1CompanyAddressReport(filters).run()

class Gstr1CompanyAddressReport(object):
	...
	def run(self):
		...
		# List of companies for selected address
		companies = frappe.get_list(
			"Dynamic Link", 
			filters={
				'parent': self.filters['company_address'],
				'parenttype': 'Address',
		...
		self.gst_accounts = get_gst_accounts(self.companies)
	...
	def get_conditions(self):
		...
		for opts in (("company_address", " and company_address=%(company_address)s"),
			("from_date", " and posting_date>=%(from_date)s"),
			("to_date", " and posting_date<=%(to_date)s"),):
				if self.filters.get(opts[0]):
					conditions += opts[1]

		conditions += " and company in (%s) " % ", ".join(["'%s'" % company for company in self.companies])
...
def get_gst_accounts(companies, account_wise=False):
	'''
	Overrided `from erpnext.regional.india.utils import get_gst_accounts` method
	to get all the accounts from the list of companies
	'''
	gst_accounts = frappe._dict()
	gst_settings_accounts = frappe.get_all("GST Account",
		filters={"parent": "GST Settings", "company": ('in', companies)},
...
def get_json(filters, report_name, data):
	...
	companies = frappe.get_list(
		"Dynamic Link", 
		filters={
			'parent': filters['company_address'],
			'parenttype': 'Address',
			'link_doctype': 'Company'
		}, fields=('link_name'))
	companies = [row['link_name'] for row in companies]

	for company in companies:
		gstin = get_company_gstin_number(company)
