...
def get_additional_rows(item_dicts):
	...
	average_row = get_average_row(item_dicts, cols)
	total_row = get_total_row(item_dicts, cols)

	item_dicts.append(average_row)
	item_dicts.append(total_row)

def get_average_row(item_dicts, cols):
	row = {'label': "Average"}
	for col in cols:
		row[col] = sum(list(map(lambda d: d[col], item_dicts))) / len(item_dicts)
	return row
